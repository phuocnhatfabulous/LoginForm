...
cdc.sso = (function (self) {
  ...
  let checkCookie = (cookieName) => document.cookie.split(";").filter((item) => item.trim().startsWith(cookieName + "=")).length > 0;
  ...
  let triggerListeners = () => {
      ...
      // empty array to prevent double execution
      callbacks = [];
  };
  // End Private Methods
...
var Targeter = (function (window, undefined) {
   ...
   * @default
   */
  var TARGETABLE_CLASS_NAME_PATTERNS = TARGETABLE_CLASS_NAMES.map(function(name) {
  ...
  var PASSTHROUGH_PROPERTIES = [
    ...
    },
    {
      ...
      validate: function (input) {
        return isDefined(input.id) && isDefined(input.categoryId);
      },
      parseError: {
  ...
  var SERVICE_MANIFESTS  = {
   ...
   loadDsc: {
     ...
     services: [{
       key: "dsc",
       callback: function () {
         updateTimeline('dsc executed', 'uidEvent', {
           subtype: 'targeterCallbackExecuted',
           markPerformance: true
         });
       }
...
// UTILITY functions
  ...
  /**
    ...
    */
    var currentHostMatches = function(matchArray) {
      return (matchArray.some(function(flag) {
        return currentHost.indexOf(flag + '.') > -1
      }));
    };
   ...
   */
  var getScriptURL = function(domain, scriptUrl) {
  ...
  var updateTimeline = function (label, type, params) {
    ...
    var performanceMarkSpecified = function (params) {
      return isDefined(params.markPerformance) && params.markPerformance
    };
  ...
  var overrideConfigValues = function(config, paramKeys) {
    paramKeys.forEach(function(paramKey) {
      config = assignFromQuery(config, paramKey);
    });
...
//
   ...
   */
  var setElementId = function (el, id) {
    el.id = id;
    return id;
  };
   ...
   */
  var normalizeAccountName = function (accountName) {
    return (accountName === 'test') ? 'test' : 'prod';
  };
   ...
   */
  var persistCurrentObjectState = function (obj) {
  ...
  var getAdobeTargetOffer = function(mbox, timeout) {
    adobe.target.getOffer({
      ...
      error: function() {
        setPageOpacity('1', 'getAdobeTargetOffer (error callback)');
      }
    });
  };
   ...
   */
  var tagExistsWithId = function (id) {
  ...
  var processRegionalMboxQueue = function (timeRemainingForXhr) {
    regionalMboxQueue.forEach(function(data, index) { // iterate over and execute each call to `mboxCreate` that was previously queued
      ...
      var params = {};
      data.arguments.slice(1).forEach(function (arg) {
        var param = arg.split('=');
...
// ADOBE EVENTS
   ...
   */
  var getDomElements = function (selector) {
  ...
  var setTargetAccount = function(config) {
    ...
    if (config && config.accountName && (typeof config.accountName !== 'undefined')) {
      ...
      var isMatch = TEST_ACCOUNT_WHITELIST.some(function(flag) {
        return currentHost.indexOf(flag) > -1
      });
  ...
  var setTargetGlobalSettingsForAccount = function(config) {
    ...
    ADOBE_ACCOUNT_PARAMS.forEach(function(param) {
      window.targetGlobalSettings[param] = config[param];
    });
  };
  ...
  var applyHideMethod = function (key) {
    ...
    if (isIFramed()) {
      setTimeout(function () {
        updateElementStyles({
          source: 'applyHideMethod (isIFramed)'
        })
      }, 3000); // show regional mboxes after a brief delay (this allows VEC authors to see regional mboxes while still preventing a content jump for those pages that have been iFramed outside of the VEC)
    }
  };
...
// LOAD SEQUENCE
   ...
   */
  function deserializeJson(input) { // NOTE: `JSON.parse` can be applied to numbers and `null` so this is no guarantee that `result.value` will be an object.
    try {
  ...
  */
  ...
  var initializeTracking = function () {
    ...
    if (!ctmTrackingMethodHasBeenDefined) {
      ...
      ctm.track = function(trackObj) { // here we assign a dummy `ctm.track` function which will be overridden with the actual function when `ctm` loads
        ctm.trackQueue.push(trackObj);
      };
    }
  };
  ...
  */
  var updateElementStyles = function (params) {
    ...
    if (Targeter.isInDebugMode) {
      ...
      timelineParams.visibleRegionalMboxCount = regionalMboxes.filter(function (regionalMbox) {
        return regionalMbox.style.visibility === "visible"
      }).length;
    }
